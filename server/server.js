...
// Словарь англицизмов, которые модель регулярно подсовывает в
// recommendations / deficits, хотя промпт требует русский. Регекс ищет
// границу слова + латиницу (case-insensitive) и подставляет русский эквивалент.
// Подбор без флексий — добавляем варианты явно ("snacks", "snack").
const EN_RU_MAP = [
  // макронутриенты
  [/\bomega[-\s]?3\b/giu,           'омега-3'],
  [/\bomega[-\s]?6\b/giu,           'омега-6'],
  [/\bomega[-\s]?9\b/giu,           'омега-9'],
  [/\bprotein(s)?\b/giu,            'белок'],
  [/\bcarb(ohydrate)?s?\b/giu,      'углеводы'],
  [/\bfats?\b/giu,                  'жиры'],
  [/\bfiber\b/giu,                  'клетчатка'],
  [/\bsugar(s)?\b/giu,              'сахар'],
  [/\bcalor(ie|ies)\b/giu,          'калории'],
  // витамины
  [/\bvitamin\s+a\b/giu,            'витамин A'],
  [/\bvitamin\s+c\b/giu,            'витамин C'],
  [/\bvitamin\s+d3?\b/giu,          'витамин D'],
  [/\bvitamin\s+e\b/giu,            'витамин E'],
  [/\bvitamin\s+k\b/giu,            'витамин K'],
  [/\bvitamin\s+b\s*1\b/giu,        'витамин B1'],
  [/\bvitamin\s+b\s*2\b/giu,        'витамин B2'],
  [/\bvitamin\s+b\s*6\b/giu,        'витамин B6'],
  [/\bvitamin\s+b\s*12\b/giu,       'витамин B12'],
  [/\bfolate\b/giu,                 'фолиевая кислота'],
  [/\bniacin\b/giu,                 'ниацин'],
  [/\bbiotin\b/giu,                 'биотин'],
  // минералы
  [/\bcalcium\b/giu,                'кальций'],
  [/\biron\b/giu,                   'железо'],
  [/\bzinc\b/giu,                   'цинк'],
  [/\bmagnesium\b/giu,              'магний'],
  [/\bpotassium\b/giu,              'калий'],
  [/\bsodium\b/giu,                 'натрий'],
  [/\bphosphorus\b/giu,             'фосфор'],
  [/\biodine\b/giu,                 'йод'],
  [/\bselenium\b/giu,               'селен'],
  [/\bcopper\b/giu,                 'медь'],
  [/\bmanganese\b/giu,              'марганец'],
  [/\bchromium\b/giu,               'хром'],
  [/\bsilicon\b/giu,                'кремний'],
  [/\bfluoride?\b/giu,              'фтор'],
  // бытовые
  [/\bsmoothie\b/giu,               'смузи'],
  [/\bsnack(s)?\b/giu,              'перекус'],
  [/\bbreakfast\b/giu,              'завтрак'],
  [/\blunch\b/giu,                  'обед'],
  [/\bdinner\b/giu,                 'ужин'],
  [/\bsupplements?\b/giu,           'добавки'],
  [/\bportion(s)?\b/giu,            'порция'],
  [/\bserving(s)?\b/giu,            'порция'],
  [/\bdairy\b/giu,                  'молочные продукты'],
  [/\bgluten\b/giu,                 'глютен'],
  [/\bglycemic\s+index\b/giu,       'гликемический индекс'],
  // хеджи + связки
  [/\bapproximately\b/giu,          'примерно'],
  [/\babout\b/giu,                  'около'],
  [/\binstead\s+of\b/giu,           'вместо'],
  [/\binclude\b/giu,                'включить'],
  [/\badd\b/giu,                    'добавить'],
  [/\breduce\b/giu,                 'сократить'],
  [/\bavoid\b/giu,                  'избегать'],
  [/\bdaily\b/giu,                  'ежедневно'],
  [/\bper\s+day\b/giu,              'в день'],
  // источники
  [/\bsource(s)?\b/giu,             'источник'],
  [/\bdeficien(t|cy)\b/giu,         'дефицит'],
  [/\bexcess\b/giu,                 'избыток'],
];

function translateEnglish(t) {
  for (const [re, ru] of EN_RU_MAP) t = t.replace(re, ru);
  return t;
}
...
function sanitizeText(s) {
  ...
  // Хеджи "примерно как", "похоже на" — убираем неуверенность
  t = t.replace(/\b(примерно|приблизительно|похоже)\s+(как|на)\s+/giu, '');
  // Перевод популярных англицизмов на русский
  t = translateEnglish(t);
...
function buildSystemPrompt() {
  return [
    ...
    '',
    '=== ЯЗЫК: ТОЛЬКО РУССКИЙ ===',
    'В полях recommendations, deficits, imbalances, product, detail — НИ ОДНОГО',
    'английского слова. Запрещено писать "omega-3" → пиши "омега-3".',
    'Запрещено "vitamin D" → "витамин D" (буква латинская допустима только',
    'как маркер витамина: A, B1, B2, B6, B12, C, D, E, K). Запрещено',
    '"snacks/breakfast/lunch/dinner/smoothie/include/avoid/reduce/per day" —',
    'все эти слова есть на русском. Латинские названия минералов (calcium,',
    'iron, zinc и т. д.) — ВСЕГДА переводи: кальций, железо, цинк, магний,',
    'калий, натрий, фосфор, йод, селен, медь, марганец, хром, кремний, фтор.',
    'Единицы — мг/мкг/г/ккал кириллицей.',
    '',
    '{',
    '  "meals": {',
    '    "breakfast": [ {',
